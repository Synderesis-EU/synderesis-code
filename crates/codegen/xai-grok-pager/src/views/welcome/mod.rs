...
pub(super) enum VersionBadgeMode<'a> {
    /// Full badge: team | tier | api_key | **Synderesis Code** VERSION+channel (right-aligned).
    Full { subscription_tier: Option<&'a str> },
    /// Hero footer: team | api_key | channel (right-aligned, gray).
    HeroFooter,
    /// Hero inline: **Synderesis Code**  VERSION (left-aligned).
    HeroInline,
}
...
pub fn render_welcome(
    ...
    let mut result = match params.auth_state {
        AuthState::Pending { error } => {
            let label = params.login_label.unwrap_or("synderesis.eu");
        ...
        AuthState::Done if params.is_zdr_blocked => {
            ...
            let (menu_rects, post_flush_escapes) = render_welcome_blocked(
                ...
                buf,
                Some((
                    "Synderesis Code is not yet available for this account.",
                    theme.gray_bright,
                )),
                &menu,
                params.selected,
...
fn render_welcome_trust(
    ...
    let lines = vec![
        ...
        Line::from(Span::styled(
            "Synderesis Code may run or modify contents in this directory,",
            Style::default().fg(theme.gray),
        ))
        .alignment(Alignment::Center),
        Line::from(Span::styled(
...
fn render_welcome_done(
    ...
    let (cursor_pos, post_flush_escapes) = if show_picker {
        ...
        let gate_text = p
            ...
            .unwrap_or("Synderesis subscription required");
        let msg = Line::from(Span::styled(
            gate_text,
            Style::default().fg(theme.gray_bright),
        ))
        ...
        if centered.height > 2 {
            ...
            let gate_link = p
                ...
                .unwrap_or("https://www.synderesis.eu/account/");
...
mod tests {
    ...
    fn version_badge_carries_no_release_label() {
        ...
        assert!(full.contains("Synderesis Code"), "full badge: {full:?}");
        assert!(inline.contains("Synderesis Code"), "inline badge: {inline:?}");
